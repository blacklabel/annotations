...
function getTransform(t) {
				if(t) {
						var tx = t.split(','),
								x = parseFloat(tx[0].substring(10)),
								y = parseFloat(tx[1]);
						return {
							x: x,
							y: y
...
Annotation.prototype = {
        ...
        render: function (redraw) {
                ...
                if(allowDragX || allowDragY) {
												$(group.element).on('mousedown', storeAnnotation);
												addEvent(chart.container, 'mousemove', translateAnnotation);
												//addEvent(chart.container, 'mouseleave', releaseAnnotation); TO BE OR NOT TO BE?
												addEvent(chart.container, 'mouseup', releaseAnnotation);
                }
                ...
                function destroyAnnotation(event) {
                	annotation.destroy();	
                }
                
                function translateAnnotation(event){
                ...
                function storeAnnotation(event) {
                		event.stopPropagation();
										chart.activeAnnotation = annotation;
										chart.activeAnnotation.startX = event.pageX;
										chart.activeAnnotation.startY = event.pageY;
                }
                
                function releaseAnnotation(event){
										if(chart.activeAnnotation) {
											var note = chart.activeAnnotation,
													xy = getTransform(note.group.transform);
